...
declare  @searchString [nvarchar](1000)=''--'13726280906chen2012@Multe2018@100122133@Qalibaab@88'
declare  @useInflectional [bit]=0
declare  @useThesaurus [bit]=0
      ...
      ELSE --base case, plain CONTAINSTABLE  
            BEGIN  
                  ...
                  FROM [HumanResources].[JobCandidate] AS FT_TBL   
                        INNER JOIN
           CONTAINSTABLE([HumanResources].[JobCandidate],*,@string,LANGUAGE @language)
           AS KEY_TBL  
                   ON  FT_TBL.[JobCandidateID] =KEY_TBL.[KEY]  
            END  
            ...
            SELECT FT_TBL.[JobCandidateID],*
                  FROM [HumanResources].[JobCandidate] AS FT_TBL 
   --≤‚ ‘CONTAINSTABLE               
   --declare @string varchar(10)=8
   --declare @language[int]=0
   --select @string,@language
   --select * from CONTAINSTABLE([HumanResources].[JobCandidate],*,@string,LANGUAGE @language)
